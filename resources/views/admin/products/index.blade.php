...
if ($stock <= 5) return ['bg'=> 'bg-amber-50', 'text' => 'text-amber-600', 'label' => 'Bajo: ' . $stock];
    ...
    @php
        ...
        $filterCount = collect(['category','status','stock','featured'])->filter(fn($f) => request($f) !== null && request($f) !== '')->count();
    @endphp
        ...
        <div class="filter-panel">
            ...
            <div class="p-4">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div class="filter-field">
                        <label>Stock</label>
                        <select id="filter_stock" onchange="applyFilters()">
                            <option value="">Todos</option>
                    ...
                    <div class="filter-field">
                        ...
                        <select id="filter_featured" onchange="applyFilters()">
                            <option value="">Todos</option>
                ...
                @if($hasFilters)
                <div class="mt-3">
                    <a href="{{ route('admin.products.index') }}" class="inline-flex items-center gap-2 px-4 py-2 text-xs font-semibold text-red-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition">
                    ...
                    if ($last <= 5) { $pages=range(1, $last); }
                        ...
                        for ($i = max(2, $current - 1); $i <= min($last - 1, $current + 1); $i++) { $pages[]=$i; }
                            ...
                            @if($products->currentPage() < $last - 1)
                                <a href="{{ $products->url($last) }}" class="w-8 h-8 flex items-center justify-center rounded-md text-gray-500 hover:bg-gray-100 transition text-xs" title="Ultima" aria-label="Última página">
